...
// Find and traverse for encoding
void find_and_traverse(Node* root, char* str, int top, char ch, FILE* outfile) {
    if (root->left) {
        str[top] = '0';
        find_and_traverse(root->left, str, top + 1, ch, outfile);
    }
    if (root->right) {
        str[top] = '1';
        find_and_traverse(root->right, str, top + 1, ch, outfile);
    }
    if (!root->left && !root->right) {
        str[top] = '\0';
        if (root->symbol[0] == ch) {
...
int main() {
    ...
    FILE* outfile = fopen("compressed.txt", "a");
    if (!outfile) {
        printf("Error opening file\n");
        return 1;
    }
    ...
    while ((ch = fgetc(file2)) != EOF) {
        find_and_traverse(root, str, 0, ch, outfile);
    }
    fclose(outfile);
